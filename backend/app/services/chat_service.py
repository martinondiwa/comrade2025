...
class ChatService:
    ...
    def get_conversation(self, user1_id: int, user2_id: int, limit: int = 50, offset: int = 0) -> list:
        """
        Fetch recent messages between two users, ordered by timestamp.
        """
        ...
        return [
            ...
            for msg in reversed(messages)  # reverse to return chronological order
        ]

    def mark_as_read(self, message_id: int, reader_id: int) -> dict:
        """
        Mark a specific message as read (optional future feature).
        """
